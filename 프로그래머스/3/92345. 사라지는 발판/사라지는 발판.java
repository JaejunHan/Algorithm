...
class Solution {
    ...
    public static int solution(int[][] arr, int[] aloc, int[] bloc) {
        board = arr;
        N = board.length;
        M = board[0].length;
 
        turn res = dfs(aloc, bloc, true, 0);
 
        return res.cnt;
    }
 
    private static turn dfs(int[] aloc, int[] bloc, boolean isATurn, int cnt) {
        int ay = aloc[0];
        int ax = aloc[1];
        int by = bloc[0];
        int bx = bloc[1];
        ...
        for (int d = 0; d < 4; d++) {
            ...
            canGo = true;
            if (isATurn) {
                res = dfs(new int[]{ny, nx}, bloc, !isATurn, cnt + 1);
            } else {
                res = dfs(aloc, new int[]{ny, nx}, !isATurn, cnt + 1);
            }
